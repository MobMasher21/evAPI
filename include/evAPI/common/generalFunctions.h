 ...
 */
template <typename T>
inline T toRadians(T deg) { return deg * DEG_TO_RAD; }
 ...
 */
template <typename T>
inline T toDegrees(T rad) { return rad * RAD_TO_DEG; }
 ...
 */
template <typename T>
inline T map(T value, T fromLow, T fromHigh, T toLow, T toHigh) { return (value - fromLow) * (toHigh - toLow) / (fromHigh - fromLow) + toLow; }
 ...
 */
vex::controller& typeToController(vex::controllerType type);
 ...
 */
buttonStatus getButtonStatus(vex::controllerType type, controllerButtons targetButton);
 ...
 */
template <typename T>
evErrorBool isCorrectDeviceInPort(T device) {
