...
namespace evAPI
{
  ...
  //Refactor Code
  void goodUI::addButtonCore(int id) {
    if(buttonList[id] != nullptr) //Returns if the button has already been created.
    { return; }

    int pgIndex;
    int xIndex;
    buttonList[id] = new button(id, buttonMode);
    pgIndex = id % 8;
    xIndex = pgIndex % 4;
    if(pgIndex >= 0 && pgIndex <= 3) {
      buttonList[id]->setDims(((40 * (xIndex + 1)) + xIndex * 70), 20, 70, 70);
    } else if(pgIndex >= 4 && pgIndex <= 7) {
      buttonList[id]->setDims(((40 * (xIndex + 1)) + xIndex * 70), 115, 70, 70);
    }
  ...
  int goodUI::addButton(int id, int r, int g, int b)
  {
    goodUI::addButtonCore(id);
    buttonList[id]->setColor(r, g, b);
    return(buttonCount);
  }

  int goodUI::addButton(int id, color buttonColor)
  {
    goodUI::addButtonCore(id);
    buttonList[id]->setColor(buttonColor);
    return(buttonCount);
  }

  int goodUI::addButton(int id, allianceType alliance)
  {
    goodUI::addButtonCore(id);

    buttonList[id]->setButtonAlliance(alliance);

    switch(alliance)
    {
      case blueAlliance:
        buttonList[id]->setColor(blue);
        break;

      case redAlliance:
        buttonList[id]->setColor(red);
        break;

      default:
        buttonList[id]->setColor(vexClrGray);
        break;
    }
  ...
  bool goodUI::addIcon(int id, bool iconInput[35][35])
  {
  ...
  bool goodUI::addTitle(int id, const char Title[MAX_TITLE_LENGTH])
  {
    if(buttonList[id] == nullptr) //Returns true if the button doesn't exist.
    { return true; }
  ...
  bool goodUI::addDescription(int id, const char Description[MAX_DESCRIPTION_LENGTH])
  {
    if(buttonList[id] == nullptr) //Returns true if the button doesn't exist.
    { return true; }
  ...
  bool goodUI::addAlliance(int id, allianceType alliance)
  {
    if(buttonList[id] == nullptr) //Returns true if the button doesn't exist.
    { return true; }
  ...
  void goodUI::pressed() {
    ...
    if(debugMode)
    { printf("xPos = %i, yPos = %i, button = %i/n", pressPointX, pressPointY, progMode); }
    ...
    if(currentMatchState == disabled) //Auto UI Code
    {
      ...
      {
        while(buttonList[i + buttonOffset] == nullptr && buttonOffset < 8)
        { buttonOffset++; }

        if(i + buttonOffset > ((activeAutoPage + 1) * 8 - 1))
        { break; }

        if(i < buttonCount)
        { buttonList[i + buttonOffset]->pressed(pressPointX, pressPointY); }
    ...
    if(buttonList[progMode]->getDescriptionLength() != 0) //Checks if there is a description.
    {
      ...
      while(true)
      {
        ...
        if(descriptionLength <= 48) //If it is less than 1 row, just print it all.
        {
          ...
          {
            if(Description[i] == '/n') //Check for new line
            { Brain.Screen.newLine(); }
  ...
  void goodUI::printButtons() {
    ...
    uint8_t buttonOffset = 0;
    for(int i = activeAutoPage * 8; i <= (activeAutoPage + 1) * 8 - 1; i++)
    {
      if(i < buttonCount)
      {
        while(buttonList[i + buttonOffset] == nullptr)
        {
          if(buttonOffset < 8)
          { buttonOffset++; }
        ...
        if((i + buttonOffset) == progMode)
        { buttonList[i + buttonOffset]->drawButton(white); }

        else
        { buttonList[i + buttonOffset]->drawButton(); }
      }
    }
