...
void AutoSelector::setDefaultColor(color colorInput) {  // Sets the default color of buttons
  defaultColor = colorInput;
}

int AutoSelector::addButton(int ID) {  // Adds a default button with the set ID and default color. Returns button count
  addButton(ID, defaultColor);
}
...
void AutoSelector::setButtonIcon(int ID, const bool (*icon)[35]) {  // Changes the icon array of the button
  if(doesButtonExist(ID)) {
...
void AutoSelector::printButtons() {  // Prints all the buttons to the screen
  ...
  int headPos = 0;
  for(int i = selectedPage * 8; i < (selectedPage * 8); i++) {
...
void AutoSelector::threadFunction( void ) {  // Function that is run in the thread
  while(1) {
